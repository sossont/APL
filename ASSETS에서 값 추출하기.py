...
for riskdata in data['RISK_V2']: # 'ASSETS_VAL_ 이라는 문자열을 만들어놓고 6~10 변수로 넣을 수 있는데, 이게 직관적이니까.
    ...
    val6list.append(int(val6))  # 문자열이어서 int로 바꿔줌.
...
# ===================== 드디어 json으로 데이터 가공 해결 ===========================

dict_v2 = str(data['RISK_V2'].loc[0])  # 계속 오류났던 이유 : 앞에 인덱싱 번호가 존재해서 계속 오류가 났던것.. loc함수로 추출해주면 인덱싱 번호 안나옴.
dict_v2 = dict_v2.replace("'","\"")
json_string = json.loads(dict_v2)
json_df = json_normalize(json_string)
print(json_df['WEEKNESS_VAL_4'])